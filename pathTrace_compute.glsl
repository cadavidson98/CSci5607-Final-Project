...
void rayRecurse(in vec3 pos, in vec3 dir, in int depth, out vec3 color);
void rayRecurse2(in vec3 pos, in vec3 dir, in int depth, out vec3 color);
void rayRecurse3(in vec3 pos, in vec3 dir, in int depth, out vec3 color);
void rayRecurse4(in vec3 pos, in vec3 dir, in int depth, out vec3 color);
void rayRecurse5(in vec3 pos, in vec3 dir, in int depth, out vec3 color);
...
vec3 sampleHemisphereCosine(float u1, float u2){
    ...
    return vec3(x, sqrt(max(0.0f, 1 - u1)), z);
};
...
vec3 sampleAroundNormalCosine(vec3 normal, vec3 bitangent, vec3 tangent){
  float r1 = rand01();
  float r2 = rand01();;
...
void rayRecurse(in vec3 pos, in vec3 dir, in int depth, out vec3 color) {
  ...
  if (hit.hit) {
    ...
    clr = clr.rgb + clr_diffuse + clr_reflective + clr_refractive;
  }
  color = clr;
}
void rayRecurse2(in vec3 pos, in vec3 dir, in int depth, out vec3 color) {
  ...
  if (hit.hit) {
    ...
    clr = clr + clr_diffuse + clr_reflective + clr_refractive;
  }
  color = clr;
}
void rayRecurse3(in vec3 pos, in vec3 dir, in int depth, out vec3 color) {
